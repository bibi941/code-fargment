...
describe('深拷贝',() => {
  ...
  it('可以复制基本类型',() => {
    // number
    const n1 = 1
    const n2 = new deepClone().clone(n1)
    assert(n1 === n2)
    // string
    const s1 = 'bibi'
    const s2 = new deepClone().clone(s1)
    assert(s1 === s2)
    // bool
    const b1 = false
    const b2 = new deepClone().clone(b1)
    assert(b1 === b2)
    // null
    const empty = null
    ...
    // undefined
    const u1 = undefined
    const u2 = new deepClone().clone(u1)
    assert(u1 === u2)
    // symbol
    const sy1 = Symbol()
    const sy2 = new deepClone().clone(sy1)
    assert(sy1 === sy2)
  })
  describe('对象',() => {
    it('能够复制普通对象',() => {
    ...
    it('能够复制函数',() => {
      ...
      const fn2 = new deepClone().clone(fn1)
      assert(fn1 !== fn2)
